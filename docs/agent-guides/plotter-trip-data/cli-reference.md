...
`recalculate-failed-routes` retries transient provider rate limits internally. Its full output reports `failedRoutesBefore`, `failedRoutesAfter`, and one `recalculatedRoutes` entry per attempted leg with `routeLegId`, `originName`, `targetName`, and final `status`; any stable provider failure remains in `routeLegs` with its human-readable `error` and structured `providerDiagnostic`. The diagnostic contains `provider`, `httpStatus`, `providerMessage`, and requested/actual profile fields when available; optional ORS detail fields are `code`, `coordinateIndex`, `retryAfterMs`, `attempts`, and `retryAttempts`. Run `audit` afterward: its `FAILED_ROUTE_LEG` issue preserves the same `providerDiagnostic` object plus the leg ID and `origin` and `target` IDs, names, coordinates, resolved labels, and source providers when available. Generic non-provider failures omit `providerDiagnostic`.
