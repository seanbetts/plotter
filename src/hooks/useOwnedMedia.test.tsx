...
import type { OwnedMediaRepository } from './useOwnedMedia';
...
describe('useOwnedMedia', () => {
  it('loads owned media and populates state', async () => {
    const media = [createMediaItem('media-1', 0)];
    const repository = createOwnedMediaRepository({
      list: vi.fn().mockResolvedValue(media),
    });

    const { result } = renderHook(() => useOwnedMedia({ ownerId: 'destination-1', repository, migrationErrorNeedle: 'media_assets.sort_order' }));
  ...
  it('clears media and avoids repository calls when owner is null', async () => {
    const repository = createOwnedMediaRepository();

    const { result } = renderHook(() => useOwnedMedia({ ownerId: null, repository, migrationErrorNeedle: 'media_assets.sort_order' }));
  ...
  it('ignores stale load responses from a previous owner', async () => {
    ...
    const { result, rerender } = renderHook(
      ({ destinationId }) => useOwnedMedia({ ownerId: destinationId, repository, migrationErrorNeedle: 'media_assets.sort_order' }),
  ...
  it('filters invalid uploads, appends valid uploads, and toggles uploading state', async () => {
    ...
    const repository = createOwnedMediaRepository({
      list: vi.fn().mockResolvedValue([createMediaItem('existing-media', 0)]),
    ...
    expect(repository.upload).toHaveBeenCalledTimes(1);
  ...
  it('normalizes valid images before uploading them to the repository', async () => {
    ...
    const repository = createOwnedMediaRepository({
      ...
      upload: vi.fn().mockResolvedValue(createMediaItem('uploaded-media', 0)),
    });
  ...
  it('updates one media item and returns the updated item', async () => {
    ...
    expect(repository.update).toHaveBeenCalledWith('media-2', {
  ...
  it('restores previous order and surfaces an error when reorder fails', async () => {
    ...
    const repository = createOwnedMediaRepository({
      list: vi.fn().mockResolvedValue(existing),
      reorder: vi.fn().mockRejectedValue(new Error('Unable to reorder media.')),
    });
...
function createOwnedMediaRepository(overrides: Partial<OwnedMediaRepository> = {}) {
  const repository = {
    list: vi.fn().mockResolvedValue([]),

    upload: vi.fn(async () => createMediaItem('uploaded-media', 0)),

    update: vi.fn(async (mediaId: string, patch: Pick<Partial<MediaItem>, 'caption' | 'credit'>) => ({
    ...
    reorder: vi.fn(async (orderedMediaIds: string[]) =>
      orderedMediaIds.map((id, index) => createMediaItem(id, index)),
    ),
  } satisfies OwnedMediaRepository;
