...
import { createTripRepository, type TripRepository } from './tripRepository';
...
export async function createAppTripRepository(options: CreateAppTripRepositoryOptions = {}) {
  ...
  const tripStorageMode = options.tripStorageMode ?? import.meta.env.VITE_TRIP_STORAGE ?? 'supabase';

  if (!isSupabaseConfigured || tripStorageMode === 'local') {
